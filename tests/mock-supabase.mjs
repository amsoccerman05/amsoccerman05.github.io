...
const server=createServer(async(req,res)=>{
 ...
 let body='';for await(const c of req)body+=c;const input=body?JSON.parse(body):{};
 ...
 try{
  ...
  const table=url.pathname.split('/').at(-1);if(!['profiles','inventory_items','inventory_events','areas','locations','categories'].includes(table))throw new Error('Unknown table');
  const values=[],clauses=[];for(const [key,val]of url.searchParams)if(['id','updated_at','inventory_item_id','area_id'].includes(key)&&val.startsWith('eq.')){values.push(val.slice(3));clauses.push(`${key}=$${values.length}`);}
  let sql;
  if(req.method==='PATCH'&&table==='profiles'){
   const sets=[];for(const key of ['display_name','role','primary_area_id','active'])if(key in input){values.push(input[key]);sets.push(`${key}=$${values.length}`);}
   sql=`update public.profiles set ${sets.join(',')} ${clauses.length?'where '+clauses.join(' and '):''} returning *`;
  }else{
