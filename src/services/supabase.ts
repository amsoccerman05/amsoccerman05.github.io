...
export const supabase=(()=>{if(demoMode||configurationError)return null;try{return createClient(url!,key!,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:new URLSearchParams(window.location.hash.slice(1)).has('access_token')?'implicit':'pkce'}});}catch{configurationError='The Supabase project URL or public key is invalid. Check the build environment configuration.';return null;}})();
